...
public class ModelAndTextureComponent extends GraphicalComponent {
    ...
    public void render(Scene scene) {
        ...
        if (entityModel.getRawModel() != null) {
            ...
            List<RenderEntity> batch = scene.getEntities().get(entityModel);
            RenderEntity thisEntity = new RenderEntity();
            thisEntity.setPosition(getEntityPosition());
            thisEntity.setRotation(getEntityRotation());
            thisEntity.setScale(scale);
            if (batch != null) {
                batch.add(thisEntity);
            } else {
                List<RenderEntity> newBatch = new ArrayList<>();
                newBatch.add(thisEntity);
    ...
    public void loadModel(Entity owner) {
        try {
            ...
            setScale((float) tree.get("scale").getDoubleValue());
