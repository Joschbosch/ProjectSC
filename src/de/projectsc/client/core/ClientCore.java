...
import static de.projectsc.client.core.data.messages.GUIMessageConstants.START_GAME;
...
import de.projectsc.client.core.algorithms.MapGenerator;
...
public class ClientCore implements Runnable {
    ...
    private void workGUI() {
        try {
            ...
            } else if (msg.getMessage().contains(CLOSE_DOWN)) {
                guiOutgoingQueue.offer(new GUIMessage(CLOSE_DOWN, null));
                networkOutgoingQueue.offer(new NetworkMessage(CLOSE_DOWN, null));
                shutdown = true;
