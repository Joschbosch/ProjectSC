...
/**
 * Represents the 3D cuboidal area of the world in which objects will cast shadows (basically represents the orthographic projection area
 * for the shadow render pass). It is updated each frame to optimise the area, making it as small as possible (to allow for optimal shadow
 * map resolution) while not being too small to avoid objects not having shadows when they should. Everything inside the cuboidal area
 * represented by this object will be rendered to the shadow map in the shadow render pass. Everything outside the area won't be.
 * 
 * @author Karl TM
 *
 */
public class ShadowBox {
    ...
    /**
     * Creates a new shadow box and calculates some initial values relating to the camera's view frustum, namely the width and height of the
     * near plane and (possibly adjusted) far plane.
     * 
     * @param lightViewMatrix - basically the "view matrix" of the light. Can be used to transform a point from world space into "light"
     *        space (i.e. changes a point's coordinates from being in relation to the world's axis to being in terms of the light's local
     *        axis).
     */
    ...
    /**
     * Updates the bounds of the shadow box based on the light direction and the camera's view frustum, to make sure that the box covers the
     * smallest area possible while still ensuring that everything inside the camera's view (within a certain range) will cast shadows.
     */
    protected void update(Camera cam) {
        ...
        toNear.scale(GUIConstants.NEAR_PLANE);
    ...
    /**
     * Calculates the center of the "view cuboid" in light space first, and then converts this to world space using the inverse light's view
     * matrix.
     * 
    ...
    /**
     * Calculates the position of the vertex at each corner of the view frustum in light space (8 vertices in total, so this returns 8
     * positions).
     * 
    ...
    /**
     * Calculates the width and height of the near and far planes of the camera's view frustum. However, this doesn't have to use the
     * "actual" far plane of the view frustum. It can use a shortened view frustum if desired by bringing the far-plane closer, which would
     * increase shadow resolution but means that distant objects wouldn't cast shadows.
     */
    private void calculateWidthsAndHeights() {
        farWidth = (float) (SHADOW_DISTANCE * Math.tan(Math.toRadians(GUIConstants.FOV)));
        nearWidth = (float) (GUIConstants.NEAR_PLANE
            * Math.tan(Math.toRadians(GUIConstants.FOV)));
